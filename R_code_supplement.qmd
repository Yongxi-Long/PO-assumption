...
This R code Supplement details all three simulation studies we describe in the main paper and reproduces tables and figures. In this Knitted PDF version, we hide some code chunks for readability, if you would like to run the whole script, you can find a complete Quarto Markdown file along with simulation results on <https://github.com/Yongxi-Long/PO-assumption>.
...
It can take several hours to run. The simulation results are available on GitHub under the folder simulation_results titled "simstudy1_comparison_shift_tests_n20.RData" for sample size 20 and "simstudy1_comparison_shift_tests_n40.RData" for sample size 40.
...
save(pvals_list,file = "simulation_results/simstudy3_pretesting_uniform_null.RData")
```
...
load("simulation_results/simstudy3_pretesting_uniform_null.RData")
ns = c(25,50,100)
...
```{r}
mod_PO_MRCLEAN <- vglm(mRS6 ~group, data=df_MR_CLEAN,
                 family = cumulative(parallel = TRUE))
...
```{r}
POA.LRT <- Perform_test(y0=df_MR_CLEAN$mRS6[df_MR_CLEAN$group==0],
                             y1=df_MR_CLEAN$mRS6[df_MR_CLEAN$group==1],
                             test.type = "POA_LRT",
                        detail = TRUE)
```
...
ORs_df$label <- factor(ORs_df$label,levels=1:6,
                       labels = c(paste0("mRS <= ",1:5),"common OR"))
...
plot.MR.CLEAN <- ggplot() +
  geom_pointrange(data=ORs_df[1:11,],
